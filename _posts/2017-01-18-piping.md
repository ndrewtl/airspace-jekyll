...
`Dplyr` is powerful package that is constantly evolving and adding new and exciting functionalities. Here are some new ways you can handle your data.

__Filtering across multiple columns__
...
```
This will return a data frame with all records that contain 8.4 in any of the columns. If you wanted to see if there were any records that contained 8.4 in every columns, you could change `any_vars` to `all_vars`. 

__Filtering for multiple criteria at the same time__
...
This would return a data frame with data regarding the "Control" and "Vaccinium" treatment types. However, since you are filtering within the same column, you don't need to keep repeating yourself! See the great shortcut below.
...
This can be computed for many different functions and conditions i.e. `is.integer`. See <a href="https://suzan.rbind.io/2018/04/dplyr-tutorial-4/#summarise-all-1">here</a> for more information.

__Renaming and reordering factors__

If you want to rename levels of a factor such as the Species in `germination`, you can employ the following: 
...
__Joining two datasets together__

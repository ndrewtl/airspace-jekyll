...
Welcome to this tutorial on INLA, written by Greg Albery off of the Pemberton Group, Institute of Evolutionary Biology, University of Edinburgh. I'm currently in the last year of my PhD working on helminth parasites in wild red deer on the Isle of Rum, my email is gfalbery@gmail.com, and my twitter handle is @Gfalbery.

Spatial autocorrelation is a common problem in ecological studies. Googling it, you'll commonly come across this phrase, Tobler's first law of geography: "everything is related to everything else, but near things are more related than distant things."

This is true of objects both in space and in time, and is commonly true of both at once. However, spatial analysis is often hard, computationally intensive, and unintuitive. Adding in a temporal element is even more intimidating. INLA is a great way to deal with this.

TWO DISCLAIMERS: 

1. This tutorial will centre around the beginner-level specifics of INLA models. It will rely on a working knowledge of GLMMs, model selection methods, etc., and won't include a huge amount of complexities about the inner workings of INLA itself. I'm not an expert on INLA, but I have a sturdy working knowledge of it, and I'm super enthusiastic about its use. I also believe that a load more systems could do with some more robust spatial and spatiotemporal analyses. If we finish early (which is extremely possible) I'm happy to discuss your data, things you'd like to do, things you'd like to try, things you're worried about, problems, etc.!
...
Recommended reading: 
This github from a paper about fisheries (quite complicated code but a great exhaustive example): https://github.com/GodinA/cjfas-bycatch-INLA-SPDE
...
Bayesian = uses Bayes's theorem, contrasted with frequentist. Based on inferring the probability of a set of data given the determined parameters. (involves setting a prior!)
Deterministic = comes up with the same results every time, contrasted with probabilistic (e.g. MCMC).

INLA allows a wide range of different functions: GLMM, GAMM, spatial autocorrelation, temporal autocorrelation, and spatiotemporal models. Combining this fact with its (eventual) simplicity and computational effeciency, it is becoming increasingly important in ecology. However, it is unintuitive when you're starting out and it can be hard to get going with.

# This tutorial #

I'm going to take you through an analysis step by step. This will involve:

1. Model selection in INLA
2. The components of an INLA model (nuts and bolts)
3. Setting up a spatial analysis
4. Modifications and specifications of spatial models
5. Spatiotemporal analyses (a little)

This will include a load of functions I've written to easily perform simple tasks using INLA, which I'm happy to deconstruct for you if needed. If you do use them in an analysis and find them helpful, or if you find a problem with them, please let me know!

The general setup of an INLA spatial analysis is as follows:

1. Plot and explore your data.
2. Decide on covariates.
3. Carry out model selection using DIC to reduce these covariates.
4. Run a final non-spatial model.
5. Decide on a set of spatial dependence structures.
...
phen <- c("Grid", "ID", "Easting", "Northing") # Base things we need
...
So first, we set up a full analysis using all the covariates that we reckon will influence the data.
...
Efxplot <- function(modellist, sig = TRUE, ModelNames = NULL){
  ...
  graph$Sig <- with(graph,ifelse((Lower<0&Upper<0)|(Lower>0&Upper>0),"*",NA))
...
NB: THIS IS A DEMONSTRATION NOT A SETUP FOR A PERFECT ANALYSIS.
- EXPLORE YOUR DATA. 
- DO NOT INCLUDE HIGHLY-CORRELATED COVARIATES. 
- BE CAREFUL OF OUTLIERS. 

IF YOU DON'T EXPLORE YOUR DATA THOROUGHLY THINGS CAN EASILY GO WRONG. DO NOT RELY ON THIS FUNCTION FOR ANALYSIS YOU HOPE TO PUBLISH, EVER, JUST DO IT BY HAND :)
...
So we ended up removing body condition, and food supplementation, with treatment, sex, and month remaining in the final model.

There are no P values in INLA. Importance or significance of variables can be deduced by examining the overlap of their 2.5% and 97.5% posterior estimates with zero. This is made easier by plotting them. But generally, using this measure is similar to using P values, and I prefer using DIC to compare variables' contributions to model fit rather than looking exclusively at the model estimates.
...
X <- as.data.frame(Xm[,-which(colnames(Xm)%in%c("Month7"))]) # convert to a data frame. Eliminate the base level of the first categorical variable if applicable (you will manually specify an intercept below) 
...
StackHost <- inla.stack(
  ...
  A = list(1, 1, 1, HostA), # Vector of Multiplication factors for random and fixed effects              
...
TICKETYBOO <- inla.stack(
  data = list(y = TestHosts[,resp]), # specify the response variable
  
  A = list(1, 1, 1, 1, HostA), # Vector of Multiplication factors for random and fixed effects              
...
INLADICFig(SpatialHostList, ModelNames = c("Base", "IID", "SPDE", "SPDE2"))
```

```r
ggField(I4, Mesh, Groups = NGroups) + # Notice the groups argument, using the number of unique months.
...
```r
ggField(I5, Groups = NGroups) + # Notice the groups argument, using the number of unique months.
  scale_fill_discrete(palette = "Greens") 
```

```r
INLARange(SpatialHostList[3:5], ModelNames = c("Full", "Monthly", "Monthly2"))
```
...
So, the best-fitting model is SPDE 3 (model 5). This features different spatial fields for each month, with correlation between the fields. However, this formulation only slightly improves model fit over the non-spatial model, so we shouldn't worry too much about the spatial effects we're seeing! Good news. Also, if you run the code below, you will see that the effect estimates barely differ between these models. So, even though space has an effect, the effect is small and doesn't modify our previous conclusions! Congratulations, you have a great result!
...
You can add a boundary to your mesh using INLA to better represent your study system. Here's an example with the Isle of Rum system I work on, portraying the coastline:

You can also remove areas of the mesh where e.g. your organism can't live, using the barrier functions.

...
Bayesian statistics therefore considers the probability of a set of parameter values (in this case, the probability of flipping a head or a tail) conditional on observing the data (whether or not it is actually a head or a tail). This probability is characterising subjective (or epistemic) uncertainity about the true values.

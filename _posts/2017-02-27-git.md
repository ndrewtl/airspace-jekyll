...
#### You will have a local copy (i.e. on your computer) and an online copy (on GitHub) of all the files in the repository. The GitHub workflow can be summaried by commit-pull-push.
...
Click `Clone or download` and if you are on a Windows computer, copy the HTTPS link (that's the one that automatically appears in the box).
If you have a Mac, click `Use SSH` and copy that link.
...
On some Macs, RStudio will fail to find Git. To fix this, first make sure all your work is saved then follow these instructions:

##### - Turn off the computer.
##### - When you turn it back on again, hold down `Cmd+r`. This will boot the mac into recovery mode. 
##### - When recovery mode has loaded, open a terminal window by selecting `Utilities` then `Terminal`. 
##### - In the terminal window type `csrutil disable` and press Enter. 
##### - Now restart the computer as normal. 
##### - Once you're logged on, open another terminal window by going to `Applications`, `Utilities`, `Terminal` and type `sudo mv /usr/bin/git /usr/bin/git-system`, then press Enter. 
##### - Now open RStudio and go to `RStudio/Preferences/ Git/SVN` and change `Git executable:` to `/usr/local/bin/git`. 
##### - Restart RStudio, go back to `RStudio/ Preferences/ Git/SVN` and click `Create RSA Key...`, then click `Create`, then `Close`
##### - Now open up another terminal window and type `ssh-add ~/.ssh/id_rsa`. Finally, restart RStudio one last time and try to create a git repository project like you did earlier in the above instructions. 
##### - More information can be found in the `README.txt` for the Mac git installer.

<center><img src="https://ourcodingclub.github.io/img/git_screen_shot.png" alt="Img" style="width: 1000px;"/></center>
...
__If you are making your first ever commit, clicking on `Commit` will result in an error message - git will tell you that you need to configure your username and email. This is easily done, and you only need to do it once, afterwards you can commit-pull-push at your convenience!__
...
Your repository is now ready to be imported in RStudio! Just like we did with the Coding Club repository earlier, if you are on a Windows computer, copy the HTTPS link (that's the one that automatically appears in the box). If you have a Mac, click `Use SSH` and copy that link.
...
<hr>
    <h2>Below you can find some more information regarding specific terminal commands</h2>
 <ul>
    <li>

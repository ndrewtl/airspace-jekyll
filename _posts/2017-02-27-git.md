...
On some Macs, RStudio will fail to find Git. To fix this, first make sure all your work is saved and turn off the computer. When you turn it back on again, hold down `Cmd+r`. This will boot the mac into recovery mode. 

When recovery mode has loaded, open a terminal window by selecting `Utilities` then `Terminal`. In the terminal window type `csrutil disable` and press Enter. Now restart the computer as normal. 

Now open another terminal window and type `sudo mv /usr/bin/git /usr/bin/git-system`, then press Enter. Now open `RStudio/ Preferences/ Git/SVN` and change `Git executable:` to `/usr/local/bin/git`. Then restart RStudio and try to create a git repository project again like you did in the above instructions. More information can be found in the `README.txt` for the Mac git installer.
...
<b>Version control is so much easier when your code makes sense to you and your collaborators - check out our <a href="https://ourcodingclub.github.io/2017/04/25/etiquette.html" target="_blank">Coding Etiquette tutorial</a> for our advice on how to keep your code tidy and organised! If you are keen to get your peers on board with Github as well, you can take a look at our <a href="https://ourcodingclub.github.io/2017/05/15/git-for-labs.html" target="_blank">Github for labs tutorials.</a></b>

...
#### - __Science communication and outreach__
...
- A command to run the app, this should be included at the very end of `app.R`. It tells shiny that the user interface comes from the object called `ui` and that the server information (data, plots, tables, etc.) comes from the object called `server`.
 ...
 __Delete any example code generated when you created the file, and create a basic shiny app by copying the bits of code above into your `app.R`.__
...
__Open up the finished web app and have a look at it by entering this in a new R script file in RStudio:__
...
#### - __`inputId = "genotype"` gives this input the name `genotype` which will become useful when referencing this input later in the app script.__
#### - __`label = "1\. Select genotype"` gives this input a label to be displayed above it in the app.__
#### - `choices = c("A" = "a","B" = "b", ...` gives a list of choices to be displayed in the dropdown menu (`A, B, etc.`) and the value that is actually gathered from that choice for use in the output (`a, b, etc.`).__
#### - `selected = "grey"` gives the value from the dropdown menu that is selected by default.__
...
Finally, to create a table output showing the contents of the selected genotype, create a new output object called `output$table` and use `renderTable()` to create a table:

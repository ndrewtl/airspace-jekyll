...
Have a look at these examples if you want to see what a Shiny app looks like, or if you want inspiration for your own app:

- [A gallery of basic Shiny apps](http://shiny.rstudio.com/gallery/).
- [A simple app exploring data on barley yield](https://johngodlee.shinyapps.io/barley_yield_exploration/)
- [Visualising study sites](https://gndaskalova.github.io/shrubhub-map/trialmap.html)
- [A complex app visualising carbon emissions in realtime](https://johngodlee.shinyapps.io/carbon_emissions/)
...
You can download the resources for this tutorial by heading to <a href = "https://github.com/ourcodingclub/CC-11-Shiny">the repository for this tutorial</a>. You can click on `Clone / Download` and either download the zip file and extract the files, or fork the repository to your own Github account. <a href = "https://ourcodingclub.github.io/2017/02/27/git.html">See our Git and Github tutorial for more info.</a>
...
- A command to run the app, this should be included at the very end of `app.R`. It tells shiny that the user interface comes from the object called `ui` and that the server information (data, plots, tables, etc.) comes from the object called `server`.
 ...
 __Delete any example code generated when you created the file, and create a basic Shiny app by copying the bits of code above into your `app.R`. Your Shiny app script should now look like this:__
...
__[Open up the finished web app and have a look at it](https://johngodlee.shinyapps.io/barley_yield_exploration/)__ 
...
Looking at the app and comparing to the panel layout examples above we can see that the app has a `sidebarLayout` with a `sidebarPanel`, `mainPanel` and `titlePanel`. It uses a `selectInput` to choose the genotype of barley shown in the histogram and the table, another `selectInput` for the colour of the histogram, a `sliderInput` to choose the number of bins in the histogram, and a `textInput` to display some text in the app. The histogram is located in the `mainPanel` along with a summary table of the data being shown, while the inputs are in the `sidebarPanel`.
...
As per our example app, we're going to be using `ggplot()` to create a histogram. For more information on creating plots in `ggplot2`, see our tutorials on [bsic data visualisation](https://ourcodingclub.github.io/2017/01/29/datavis.html) and [customising ggplot graphs](https://ourcodingclub.github.io/2017/03/29/data-vis-2.html). 
...
If you want to quickly share the app over the internet we recommend using [Github](github.com) to host the file.

Go to [github.com](github.com), sign in with your account details, create a repository and upload everything from your app folder, including the `Data` and `www` folders. 
...
![Img]({{ site.baseurl }}/img/github_create_new_file.jpg)
...
![Img]({{ site.baseurl }}/img/rstudio_shiny_publish.jpg)
...
![Img]({{ site.baseurl }}/img/shinyapps_io_dash.jpg)

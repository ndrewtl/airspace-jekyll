...
---
<center><img src="http://i212.photobucket.com/albums/cc93/_avocet/_avocet133/Untitled-12.jpg" style="width: 664px;"/></center>
...
The next lines of code usually describe the packages we are using in this analysis - it's convenient to keep them together at the top. To install a package, you need to type in <i>install.packages("package-name")</i>. You only need to install packages once, afterwards you just need to load them using <i>library(package-name)</i>. Here we will be using the package <a href="https://cran.r-project.org/web/packages/dplyr/index.html">dplyr developped by Hadley Wickham</a>. We will use <i>dplyr</i> a lot in the future - this package is excellent at formatting and manipulating data.
...
You're probably noticing a pattern in the code above - and a lot of copy and pasting! That's not particularly efficient - in future tutorials we will learn how to use more of <i>dplyr</i>'s functions and achieve the same result with way less code! Now we can combine all those values in a vector, which is done using the c() function, and add labels with names().

...
R is a statistical programming language that has rapidly gained popularity over the last ten years in many scientific and economic fields. The main difference between R and other statistical softwares like Minitab, SPSS and Stata is that R has no graphical user interface. There are no buttons to click and no dropdown menus, R can be run entirely by typing commands into a text interface. While this might seem daunting and technical at first, hopefully by the end of this tutorial you will see how R can help you to improve your statistical analysis. 
...
- Because R is a programming language rather than a graphical button interface like Minitab, the user can easily save scripts as small plain text files for use in the future, or share them with collaborators.
- Because of it's popularity, R has a very active and helpful online community, normally a quick google is all it takes to find somebody who has fixed the problem you're having 
...
Practice is the best way to learn any new language, so in this tutorial we will jump straight in and do some of our own statistical analysis using a publicly available dataset which includes occurrence records for many animal, plant, and fungi species. The data were collected by different NGOs and environmental agencies. We downloaded the records for 2000-2016 (from the NBN Gateway https://data.nbn.org.uk/), which you can get from this <a href="https://github.com/ourcodingclub/CC-1-RBasics" target="_blank">Github repository</a>. 

To provide some extra help in transitioning to R we will be using a program called RStudio as a graphical front-end to R. 
...
Open RStudio. Click on <i>File/New File/R script</i>. You will now see a window like the one above. You can type code directly into the console - pressing enter at the end of the line runs the code. You can also write your code in a script (e.g. in the new script file you just created) - to run a line of code from your script, press <i>Ctrl+Enter</i> on Windows and <i>Cmd+Enter</i> on a Mac. __If you enter code directly into the console it will not be saved by R, so by first typing your code into a script file you are creating a reproducible record of your analysis__. 

When writing a script, it's useful to add comments to describe your analysis by inserting a `#` in front of a line of text. Begin your script by adding some comments on who is writing the script, the date, as well as the main goal - in our case, determining how many species from different taxa have been recorded in Edinburgh. Here's an example:
...
As a convention, the next few lines of code usually describe the packages you are using for you analysis. A package is a bundle of commands that can be loaded into R, to provide extra functionality. For example you might load a package for conducting mixed effects models, or for plotting map data. 
...
setwd(C:\User\/Users/johngodlee/Downloads/CC-1-RBasics-master)
...
__R is an object-based language - this means that the data you import, the values you might create later, they are all stored in objects that you name. The arrow `<-` is how you assign objects - here we assigned our csv file to the object `edidiv`. In the top right window of RStudio you can see the names of any objects currently loaded into R.__
...
__Our `edidiv` object has occurrence records of various species collected in Edinburgh from 2000 to 2016. To explore Edinburgh's biodiversity we will create a graph showing how many species were recorded in each taxonomic group.__ 
...
The objects we created before were called vectos, which are a series of values, each with a label. This object type is suitable when dealing with just one set of values. Often however, you will have more than one variable and they might be different types of variables - e.g. some continuous, some categorical. In those cases, we will use data frame objects. __Data frames are tables of values, a two-dimensional structure with rows and columns where each column can have a different data type. Another possible data format is a matrix - a matrix can have several rows of data as well (e.g. you can combine vectors into a matrix), but the variables are all of the same type, for instance they are all numerical, and are the same length in terms of the number of rows.__

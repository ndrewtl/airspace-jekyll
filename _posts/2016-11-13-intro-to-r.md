...
---
<div class="block">
          <center><img src="{{ site.baseurl }}/img/tutheader.png" alt="Img"></center>
        </div>
...
| #### 1. Learn how to import data | #### <a href="#download">1. Download R and RStudio</a> | #### 1. You are familiar with the RStudio interface and can import data

| #### 2. Learn how to create vectors and data frames | #### <a href="#import">2. Import and check data</a> | #### 2.You can check and explore data |
...
The next lines of code usually describe the packages we are using in this analysis - it's convenient to keep them together at the top. To install a package, you need to type in `install.packages("package-name")`. You only need to install packages once, afterwards you just need to load them using `library(package-name)`. Here we will be using the package <a href="https://cran.r-project.org/web/packages/dplyr/index.html"><i>dplyr</i> developped by Hadley Wickham</a>. We will use <i>dplyr</i> a lot in the future - this package is excellent at formatting and manipulating data.
...
<b>When working in R, you have a working directory (a certain folder on your computer) - that's where anything you save, such as plots will go. To find out where your working directory is now, run the code `getwd()`. If you want to change it, you can use `setwd()`:</b>
...
Now we can import our data - in RStudio you can either click on the "Import dataset" button and navigate to where you have saved your file, or use the `read.csv()` command. A window will pop up previewing your data, make sure that next to <i>Heading</i> you've selected <i>Yes</i> (this tells R to treat the first row of your data as the column titles) and click Import. If you click on <i>Import dataset</i>, afterwards in the console you will see the code for your import, which includes the file path - it's a good idea to copy that code into your script, so that for future reference, you know from where you imported your dataset and which dataset exactly you imported. R works best with csv (comma separated values) files - if you entered your data in Excel, you would need to click on <i>Save as</i> and select csv as the file extension. A message will come up, click ok. When entering data in excel, don't put any spaces in your row names, as they will confuse R later.
...
A really important step is to check that your data imported alright - it's good practice to always run this code:
...
You might have noticed that there are spaces in front of and behind `<-`, after commas and around `==`, those are added to make it easier to read the code. Additionally, all the labels have been indented on a new line - otherwise the line of code gets very long and it's rather inconvenient to have to keep scrolling left and right to see your code. Also, take care to check that you are matching your vector values and labels correctly - you wouldn't want to label the number of beetles as lichen species! This would have been way easier to check if the objects were called e.g. `beetle_sp`, not `a` - the good thing about keeping a record of your script though is that I can go back and check that I have indeed assigned the number of beetle species to `a`.
...
This was a vector of values, each with a label, which is suitable when dealing with just one set of values. However, most often you will have more than one variable and they might be different types of variables - e.g. some continuous, some categorical. In those cases, we will use data frames - <b>data frames represent tables of values, so a two-dimensional structure with rows and columns. Another possible data format is a matrix - a matrix can have several rows of data as well (e.g. you can combine vectors into a matrix), but the variables are all of the same type, for instance they are all numerical, and of the same length - e.g. 15 rows in each column.</b>

...
| __Step__ |              __Description__            |                                 __Notes__                                       |
|:-----|:----------------------------------------|:--------------------------------------------------------------------------------|
| 1.   | __The right type of plot for your results__ | Might be a boxplot, a scatterplot, a linear regression... many options!         |
| 2.   | __The graph is well-organised__             | The independent (explanatory) variable is on the x axis, and the dependent (response) variable is on the y axis. |
| 3.   | __The x and y axis are in the right units__ | Having proper symbols (for alpha, beta, etc.) and superscript/underscript where needed makes graphs look way more professional |
| 4.   | __The x and y axis are easy to read__       | Beware awkward fonts and tiny letters!                                          |
| 5.   | __There is a clear legend__                 | It's easy to tell apart what points/lines on the graph represent.               |
| 6.   | __The graph is not cluttered__              | Don't put all your results on the same plot, give them space to shine!          |
| 7.   | __Nice and consistent colour scheme__       | Stick with the same colours for the same variables, avoid red/green combinations|
| 8.   | __The graph is the right size__             | Avoid overlapping labels and points/lines which merge together, and make your graph longer/wider if needed. |
| 9.   | __A measure of uncertainty is reported__    | Error bars, condidence and credible intervals, remember to say in the caption what they are. |
| 10.  | __Concise and informative caption__         | Remeber to include what the data points show (raw data? model predictions?), what is the sample size for each treatment, the effect size and what measure of uncertainty accompagnies it. |
...
__Figure 2. Population trends of Griffon vulture in Croatia and Italy from 1970 to 2014. Data points represent raw data with a linear model fit and 95% confidence intervals. Abundance is measured in number of breeding individuals.__

If your axis labels need to contain fancy characters \gamma or superscript, you can get `ggplot2` to plot that, too. It might require some googling regarding your specific case, but for example, this code `ylabs(expression(paste("Grain yield","  ","(ton.", ha^-1,")", sep="")))` will create a y axis with a Grain yield ton. ha^-1 label.
...
__You might be picking up on the fact that we repeat a lot of the same code - same font size, same margins, etc. Less repetition makes for tidier code, so in our next tutorial we will learn how to write our own functions and loops to create multiple plots at once!__

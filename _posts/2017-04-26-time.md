...
```r
install.packages("ggplot2")  # For plotting forecasts
install.packages("lubridate")  # Efficient data formatting
...
Data in the `Date` class in the conventional `YYYY-MM-DD` format are easier to use in `ggplot2` and various time series analysis packages. `format =` tells `as.Date` what form the original data is in. The symbols `%Y`, `%m`, `%d` etc. are codes understood by many programming languages to define date types. Here is an expanded table of date codes:
...
Now that you have worked through the tutorial use what you have learnt to make some model forecasts for our data on __ which you can find in `gev.csv` in the folder you downloaded from the 
<a href="https://github.com/ourcodingclub/CC-time-series" target="_blank">the github repository</a>.

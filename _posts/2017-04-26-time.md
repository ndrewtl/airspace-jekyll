...
Let's first compare model forecasts of different ETS models visually by extracting forecast data from `forecast` objects and plotting it in ggplot against the original data. The code below is quite verbose, and could be made more concise by using pipes, or apply() functions, but this allows you to investigate all the intermediate objects for yourself, so you understand how they are structured and what they show:

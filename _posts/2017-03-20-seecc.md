...
=======
__There is one more problem we need to solve. Some of the datapoints are not on the coast, which means that these pictures are probably not puffins. In order to delete them, we are going to use the UK coastline to select only the datapoints that are within 1Km of the coast and the ones that are on the sea.__
>>>>>>> origin/master
...
=======
If we want to select datapoints according to their distance in km from the coastline we need to transform our spatial datasets into projected coordinate systems, preferably UTM for the UK. In R it is pretty easy, in fact you only need one function.
>>>>>>> origin/master

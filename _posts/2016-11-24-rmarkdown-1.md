...
#### <a href="#what">1. Find out what R Markdown is </a>
...
__Read through this tutorial, and use the information you learn along the way to convert the tutorial R script (`RMarkdown_Tutorial.R`) which you can find in the repo, into a well commented, logically structured `.Rmd` document. Afterwards there are some challenge scripts that you can convert to `.Rmd` documents.__
...
## Download R Markdown
To get R Markdown working in RStudio the first thing you need is the `rmarkdown` package, which you can get from [CRAN](https://cran.r-project.org/web/packages/rmarkdown/index.html) using the following commands in R or RStudio:
...
Now save the `.Rmd` file to the repository you downloaded from github.

Now open the `RMarkdown_Tutorial.R` practice script from the repo you downloaded earlier and use the instructions below to help you convert this script into a coherent RMarkdown document, bit by bit.

If you have any of your own R scripts that you would like to make into an RMarkdown document, you can also use those! 

<a name="identify"></a>

## The YAML Header

At the top of any R Markdown script is a `YAML` header section enclosed by `` --- ``. By default this includes a title, author, date, and the file type you want to output to. Many other options are available for different functions and formatting, see <a href="http://rmarkdown.rstudio.com/html_document_format.html">for `.html` options</a> and <a href="http://rmarkdown.rstudio.com/pdf_document_format.html">here for `.pdf` options</a>. Rules in the header section will alter the whole document. Have a flick through quickly to familiarise yourself with the sorts of things you can alter by adding an option to the `YAML` header. 
...
By default, the `title`, `author`, `date` and `output` format are printed at the top of your `.html` document. This is the minimum you should put in your header section. To compile your `.Rmd` file into a `.html` document you should press the `Knit` button in the taskbar:

<img src="{{ site.baseurl }}/img/Knit_HTML_Screenshot.jpg" alt="Img">

 Not only does a preview appear in the `Viewer` window in RStudio but it also saves a `.html` file to your working directory.
...
You can run a chunk of code at any time by placing your cursor inside the code chunk and selecting `Run -> Run Current Chunk`:
...
If you want a bit more control over the content of your table you can use ``pander()`` in the `pander`. Imagine I want the 3rd column to appear in italics:
...
To create `.pdf` documents for printing in A4 requires a bit more fiddling around. RStudio uses another document compiling system called [LaTeX](http://www.latex-project.org) to make `.pdf` documents.

If you are using Windows go to <a href="https://miktex.org/download">MikTeX</a> and download the appropriate "Complete MikTeX Installer" for your system, either 32-bit or 64-bit.

If you are using a Mac go to <a href="https://tug.org/mactex/mactex-download.html">MacTeX</a> and download the "MacTeX.pkg".

Running these installers will install a version of LaTeX onto your system which R will then be able to call to compile the `.pdf`.

Becoming familiar with [LaTeX syntax](https://tobi.oetiker.ch/lshort/lshort.pdf) will give you a lot more options to make your R markdown `.pdf` look pretty, as LaTeX commands are mostly compatible with RMarkdown, though some googling is often required.

To compile a pdf instead of a html document, change `output:` to `pdf_document`. 
...
At this point, have a go at converting the tutorial R script (`RMarkdown_Tutorial.R`) into a `.Rmd` document using the information above as a guide.

Remember that a good R markdown document should provide a reproducible log of your code, properly commented and with subtitles, comments and code relevant code output so the reader knows what is going on.
...
Either in a small group or on your own, convert one of the three demo R scripts into a well commented and easy to follow R Markdown document, or R Markdown Notebook. The files ('RMarkdown_Demo_1.R', 'RMarkdown_Demo_2.R', 'RMarkdown_Demo_3.R') can be found in the <a href="https://github.com/ourcodingclub/CC-2-RMarkdown">repo you downloaded earlier</a>. 

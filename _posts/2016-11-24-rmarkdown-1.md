...
By default, figures are rendered as `.png` files by R Markdown, this can lead to loss of quality if your document is rescaled so you can change that to `.svg`, a vector file format by adding `dev='svg'` to the code chunk instruction section.
...
To create `.pdf` documents for printing in A4 requires a bit more fiddling around. RStudio uses another document compiling system called <a href="http://www.latex-project.org target="_blank">LaTeX</a> to make `.pdf` documents.
